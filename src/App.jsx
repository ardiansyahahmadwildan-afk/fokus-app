...
import {
  ...
  User, Search, FileDown, AlignLeft
} from "lucide-react";
...
const Modal = ({ open, onClose, title, children, wide }) => {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(6px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.card,borderRadius:16,padding:24,width:'100%',maxWidth:wide?680:460,border:`1px solid ${T.border}`,boxShadow:'0 32px 80px rgba(0,0,0,0.6)'}}>
      ...
      <div style={{width:'100%',maxWidth:380}}>
        ...
        <div style={{background:T.card,borderRadius:16,padding:28,border:`1px solid ${T.border}`}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            {msg && <p style={{color:msg.includes('Cek')?T.emerald:T.rose,fontSize:12,margin:0,padding:'8px 12px',background:msg.includes('Cek')?T.emerald+'15':T.rose+'15',borderRadius:8}}>{msg}</p>}
...
function NotesView({ user }) {
  ...
  const exportPDF = (n) => {
    ...
    if (n.tag) {
      ...
      doc.text(`Tag: ${n.tag}`, margin, y);
      y += 8;
    }
    doc.setFontSize(9);
    doc.setFont('helvetica','normal');
    doc.text(new Date(n.created_at).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'}), margin, y);
    y += 10;
    doc.setDrawColor(200);
    doc.line(margin, y, margin + pageWidth, y);
    y += 8;
    doc.setFontSize(11);
    const contentLines = doc.splitTextToSize(n.content||'', pageWidth);
    ...
    <div>
            ...
            <div key={n.id} onClick={()=>openEdit(n)} style={{background:T.card,borderRadius:12,padding:16,cursor:'pointer',border:`1px solid ${T.border}`,borderTop:`3px solid ${n.color}`,transition:'transform 0.15s,box-shadow 0.15s'}}
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                ...
                <div style={{display:'flex',gap:4,flexShrink:0}}>
                  <button onClick={e=>{e.stopPropagation();setReading(n);}} style={{background:'none',border:'none',cursor:'pointer',color:T.muted,padding:'0 0 0 4px'}} title="Mode Baca"><AlignLeft size={13}/></button>
      ...
      <Modal open={!!reading} onClose={()=>setReading(null)} title={reading?.title||''} wide>
        <div style={{maxHeight:'65vh',overflowY:'auto',paddingRight:8}}>
          {reading?.tag && (
            <span style={{fontSize:11,color:TAG_COLORS[reading.tag],background:TAG_COLORS[reading.tag]+'20',padding:'2px 10px',borderRadius:10,display:'inline-block',marginBottom:12}}>{reading.tag}</span>
          )}
          <p style={{color:T.subtle,fontSize:14,lineHeight:1.9,whiteSpace:'pre-wrap',margin:0}}>{reading?.content}</p>
          <p style={{color:T.muted,fontSize:11,marginTop:20,marginBottom:0,borderTop:`1px solid ${T.border}`,paddingTop:12}}>
            {reading && new Date(reading.created_at).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'})}
          </p>
        </div>
      </Modal>
    </div>
...
function TodoView({ user }) {
  ...
  const exportPDF = () => {
    ...
    doc.text('Daftar Tugas', margin, y); y += 12;
    doc.setFontSize(11); doc.setFont('helvetica','normal');
    todos.forEach(t => {
      const status = t.done ? '[✓]' : '[ ]';
      const lines = doc.splitTextToSize(`${status} ${t.text}`, pageWidth);
      if (y > 270) { doc.addPage(); y = 20; }
